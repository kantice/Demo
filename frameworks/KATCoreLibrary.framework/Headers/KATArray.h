...
///是否正在重组
@property(nonatomic,readonly) BOOL isRefactoring;
...
///交换成员位置
- (BOOL)changePositionWithMemberA:(T)a andMemberB:(T)b;
...
///前移成员(向前移动一位)(最多到首位)
- (void)forwardMember:(T)member;

///前移成员(最多到首位)
- (void)forwardMember:(T)member byStep:(int)step;

///后移成员(向后移动一位)(最多到末位)
- (void)backwardMember:(T)member;

///后移成员(最多到末位)
- (void)backwardMember:(T)member byStep:(int)step;
...
///某个属性数值最小的成员(可以为多级属性，如self.age，一般省略self.，也可以只用self)
- (T)smallestMemberWithKey:(NSString *)key;

///某个属性数值最大的成员(可以为多级属性，如self.age，一般省略self.，也可以只用self)
- (T)biggestMemberWithKey:(NSString *)key;
...
///成员某个属性求和(浮点数)(可以为多级属性，如self.age，一般省略self.，也可以只用self)
- (double)floatSumOfMembersWithKey:(NSString *)key;

///成员某个属性求和(整数)(可以为多级属性，如self.age，一般省略self.，也可以只用self)
- (long long int)integerSumOfMembersWithKey:(NSString *)key;
...
///通过关键词排序(返回新数组)(升序排列，若要降序，排序后反序即可)(关键词对应为数组成员的属性)(关键词权重按顺序，若元素不存在任意一关键词属性，则不参与排序)(若属性为数字，则以数值大小为基准，若为字符串，则已字符顺序为基准，其他对象属性以hashCode为基准)(注意:数字超过16位会丢失后面的精度,小数最多保留8位精度)
- (instancetype)sortByKey:(NSString *)key;

///通过关键词数组排序(返回新数组)(升序排列，若要降序，排序后反序即可)(关键词对应为数组成员的属性)(关键词权重按顺序，若元素不存在任意一关键词属性，则不参与排序)(若属性为数字，则以数值大小为基准，若为字符串，则已字符顺序为基准，其他对象属性以hashCode为基准)(注意:数字超过16位会丢失后面的精度,小数最多保留8位精度)
- (instancetype)sortByKeys:(KATArray<NSString *> *)keys;

///筛选(返回新数组)(条件格式为:属性(若自身则用self,只能是数字或字符串类型),符号(==,>=,<=,>,<,!=),参考值(若属性为数字，则参考值会转换为double类型的数字，精确6位小数，慎用等号),例如:name>=kat,age<20,self>3,son.age>=30等;若对比值为属性值,则在属性名前加&,例如:age<&money,age>&son.age等,不以&为开始则视为普通字符串;若想用以&开头的字符串,则用\&开头)(关系为'AND'或者'OR',YES则为OR,NO为AND,整个条件数组都用该关系)(为简化操作忽略所有空格,包括对比的字符串值中的空格)
- (instancetype)filterWithRelationshipOR:(BOOL)OR andConditions:(KATArray<NSString *> *)conditions;

///筛选(返回新数组)(条件格式为:属性(若自身则用self,只能是数字或字符串类型),符号(==,>=,<=,>,<,!=),参考值(若属性为数字，则参考值会转换为double类型的数字，精确6位小数，慎用等号),AND关系运算符'&&'和OR关系运算符'||'同时存在时，优先运算AND关系,不支持括号调整优先级,例如:name>=kat || age<20 && money>100.0,self>30&&son.age>=3等,若对比值为属性值,则在属性名前加&,例如:age<&money,age>&son.age等,不以&为开始则视为普通字符串;若想用以&开头的字符串,则用\&开头)(为简化操作忽略所有空格,包括对比的字符串值中的空格)
- (instancetype)filterWithCondition:(NSString *)condition;

///分组(返回KATHashMap<KATArray<T> *> *):根据属性key的取值进行分组，可以为多级属性，如self.name.firstName，一般省略self.，也可以只用self.如原数组中的key有"A","B"两种取值，则将会分为“A”,"B"两个组，若数组成员无该属性，则不会归类到任何分组中
- (KATHashMap *)groupByKey:(NSString *)key;
