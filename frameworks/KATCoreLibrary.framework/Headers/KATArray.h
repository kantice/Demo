...
///筛选(条件格式为:属性(若自身则用self,只能是数字或字符串类型),符号(==,>=,<=,>,<,!=),参考值(若属性为数字，则参考值会转换为double类型的数字，精确6位小数，慎用等号),AND关系运算符'&&'和OR关系运算符'||'同时存在时，优先运算AND关系,不支持括号调整优先级,例如:name>=kat || age<20 && money>100.0,self>30&&son.age>=3等,若对比值为属性值,则在属性名前加&,例如:age<&money,age>&son.age等,不以&为开始则视为普通字符串;若想用以&开头的字符串,则用\&开头)(为简化操作忽略所有空格,包括对比的字符串值中的空格)
